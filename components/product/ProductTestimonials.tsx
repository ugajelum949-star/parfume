...
function ReviewCard({ t, rotation }: { t: Testimonial; rotation: string }) {
  return (
    <div className={`${rotation} group hover:!rotate-0 transition-transform duration-300 shrink-0 w-[260px] md:w-[300px]`}>
      <div className="bg-card/80 backdrop-blur-sm border border-border rounded-2xl p-5 flex flex-col gap-3 hover:border-accent/30 transition-colors h-[200px]">
        ...
        {t.proofImage && (
          <img src={t.proofImage} alt="Bukti" className="w-full h-16 object-cover rounded-xl border border-border shrink-0" />
        )}
      </div>
    </div>
  )
